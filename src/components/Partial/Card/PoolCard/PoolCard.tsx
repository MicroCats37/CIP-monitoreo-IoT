...
export default function PoolCard({ data }: { data: PoolType[] }) {
  return (
    <div className="flex gap-6 w-full flex-col lg:flex-row">
      {data.map((pool, index) => {
        const status = getChlorineStatus(pool.data.cloro)
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <CardTitle className="text-2xl font-bold">{pool.data.piscina}</CardTitle>
                  <p className={cn("text-sm font-medium", status.colors.text)}>{status.message}</p>
                </div>
                <div className="relative">
                  <Droplets className={cn("h-12 w-12 transition-all duration-300 opacity-20", status.colors.text)} />
                  <StatusIcon
                    className={cn(
            ...
            <CardContent>
              <div className="mt-4 flex flex-col items-center gap-4">
                <div className="text-4xl font-bold tracking-tighter">
                  {pool.data.cloro}
                  <span className="text-base font-normal text-muted-foreground ml-1">mg/L</span>
                </div>
                <Badge
                  variant="secondary"
