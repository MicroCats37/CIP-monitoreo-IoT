...
import { ConcentracionCo2Type } from "@/validators/devices/schemas"


interface Props {
  dataMQTT: ConcentracionCo2Type;
}
...
export default function ConcentracionCo2Card({ dataMQTT }: Props) {
  return (
    <div className="flex gap-6 w-full flex-col lg:flex-row">
      {dataMQTT.details.map((location, index) => {
        const co2Value = location.data.fields.co2!
        const status = getCO2Status(co2Value)
          ...
          >
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <CardTitle className="text-2xl font-bold">{location.data.sensor.name}</CardTitle>
                  <p className={cn("text-sm font-medium", status.colors.text)}>{status.message}</p>
                </div>
                <div className="relative">
                  <Wind className={cn("h-12 w-12 transition-all duration-300 opacity-20", status.colors.text)} />
                  <StatusIcon
                    className={cn(
            ...
            <CardContent>
              <div className="mt-4 flex flex-col items-center gap-4">
                <div className="text-4xl font-bold tracking-tighter">
                  ...
                  <span className="text-base font-normal text-muted-foreground ml-1">ppm</span>
                </div>
                <Badge
                  variant="secondary"
                  className={cn("px-4 py-1 text-sm font-medium transition-all duration-300", status.colors.badge)}
                >
                  {status.message}
                </Badge>
              </div>
            ...
            <CardFooter className="border-t bg-white/50 dark:bg-black/5">
              <p className="text-sm text-muted-foreground text-center w-full pt-2">
                Última actualización: {new Date(location.time).toLocaleString()}
              </p>
            </CardFooter>
          </Card>
        )
