// app/api/co2/download/route.ts
import { NextResponse } from "next/server";
import { getCO2DownloadData } from "@/influxDB/querys-download/CO2Download";
...
export async function GET(request: Request) {
    try {
        ...
        const csv = await getCO2DownloadData(startTime, endTime);

        // Devolver el archivo CSV como respuesta
        return new NextResponse(csv, {
            headers: {
                "Content-Type": "text/csv",
                "Content-Disposition": 'attachment; filename="co2_datos.csv"',
            },
        });
