// app/api/sci/download/route.ts
import { NextResponse } from "next/server";
import { getSCIDownloadData } from "@/influxDB/querys-download/SCIDownload";
...
export async function GET(request: Request) {
    try {
        ...
        const csv = await getSCIDownloadData(startTime, endTime);

        // Devolver el archivo CSV como respuesta
        return new NextResponse(csv, {
            headers: {
                "Content-Type": "text/csv",
                "Content-Disposition": 'attachment; filename="sci_datos.csv"',
            },
        });
